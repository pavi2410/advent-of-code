...
enum class Direction(val d: Char) {
    ...
    companion object {
        private val map = values().associateBy { it.d }
        operator fun get(d: Char) = map[d] ?: throw error("unknown dir: $d")
    }
}

data class Vector(val dir: Direction, val m: Int) {
    companion object {
        fun parse(s: String): Vector {
            val dir = Direction[s.first()]
            val m = s.drop(1).toInt()
...
data class Point(val x: Int = 0, val y: Int = 0) {
    fun manDist(other: Point = Point()) = abs(x - other.x) + abs(y - other.y)
...
data class Wire(val points: List<Point>) {
    infix fun intersect(other: Wire) = points intersect other.points

    companion object {
        fun parse(path: String): Wire {
            val vectors = path.split(",").map(Vector.Companion::parse).toMutableList()
            val points = vectors.fold(mutableListOf(Point())) { a, b ->
                repeat(b.m) {
                    a += a.last().move(b.dir)
                }
                a
...
fun result(paths: Pair<String, String>): Int {
    val (path1, path2) = paths
    val wire1 = Wire.parse(path1)
    val wire2 = Wire.parse(path2)
    val intersections = wire1 intersect wire2
    println(intersections)
    return intersections.map { it.manDist() }.min() ?: 0
}
...
for ((paths, v) in testcases) {
    val dist = result(paths)
